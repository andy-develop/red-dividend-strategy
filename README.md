...
**回测结果（v7.11，2016-09-08 ~ 2026-09-08，初始 10 万）**：
策略 **+168.2% / 夏普 0.68 / 最大回撤 −31.7%**；买入持有 +134.3% / 0.62 / −27.5%。44 笔；抄底 20 档胜率 90.0%（平均 +5.6%）、加仓 2.0 次/年。
v7.10 = 超卖跌幅阈值 15→14（walk-forward 三段一致增强）；v7.11 = 年线门 + 估值门（隔离测试：纯年线 +172.3%/0.67/−30.4%、年线+估值3y +165.5%/0.67/−31.7% 胜率 90%；周线共振门/跌幅加速/REBUY45/动能T+3/顶背离三选二均回测否决，见 HANDOFF 第 16-17 节）。
（v7.6 的 +328.5%/0.97/−27.6% 基于"同日收盘成交+零成本杠杆+TR 信号"，不可复现且系统性乐观，已废弃。）

## 每日更新逻辑（update.py）与可靠性（v7.11 加固）

- **数据时效**：中证当日收盘数据延迟发布（16:56 实测仍无当日数据），早 8 点跑必然取到 **T-1 日收盘**——页面 data_date 显示昨日收盘，供当日开盘前决策，属正常设计。
- **数据校验**：update.py 每次抓取后校验——最新数据距今天 >10 天报"陈旧"警告、>3 天报"滞后"提示、非正收盘价/空值/全收益单日变动>25% 均检出；警告显示在页面顶部黄条（不静默），异常时仍生成页面避免全天空白。
- **版本锁定**：workflow 改用 `requirements.txt`（pandas>=2.0,<2.3 / numpy>=1.24,<2.3），防止 pandas 大版本升级移除弃用 API 导致任务崩溃；engine 已清除 `fillna(method=)` 弃用调用。
- **发布自检**：workflow 末尾新增"Verify published page"步骤，抓取线上页面比对 generated_at 与本地产物，不一致即标红。
- **时间戳**：页面 generated_at 统一为北京时间（UTC+8）。
- **十年期国债缓存**：cn10y_daily.csv 为静态缓存（2013-2026），国债收益率是慢变量但长期会漂移，**建议每季度手动跑一次 `python3 refresh_cn10y.py` 刷新**（需 akshare，Actions 流水线不装）；缓存缺失时引擎自动降级（估值门失效、其余正常）。
