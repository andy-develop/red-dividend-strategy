...
**v7.13 左侧目录导航**：页面左侧新增可点击的树形目录——一级「选ETF / ETF择时」；「ETF择时」下设 红利低波（唯一有真实内容，即现有仪表盘）/ 沪深300 / 中证500；「选ETF」下设 资产配置策略 / 行业轮动策略 / 估值驱动策略 / ETF分类（三级：宽基ETF·中风险 / 行业主题ETF·高风险 / 跨境ETF·高风险）。除红利低波外的所有节点为得体空页（面包屑 + 建设中说明）。hash 路由（#/timing/hongli 等），移动端侧栏折叠为左下角抽屉按钮。
